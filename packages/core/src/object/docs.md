...
```ts
const result = object({ name: "alice", age: 30, role: "admin" }).omit(["role"])
	.patch({ age: 31 }).value();
...
object(partial).defaults({ role: "user", status: "active", name: "fallback" })
	.value();
...
```ts
const result = object({ a: 1, b: 2, c: 3 }).omit(["c"]).pick(["a"]).value();
...
object(def).omit(["internal"]) // type: { name, passwordHash, role, client }
	.patch({ client: { read: true, insert: false, update: true } }) // type: { name, passwordHash, role, client } — client updated deeply
	.pick(["name", "role", "client"]) // type: { name, role, client }
	.value();
