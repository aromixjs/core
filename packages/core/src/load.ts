...
function makeDenoAdapter(): FSAdapter {
	...
	const join = (...parts: string[]) => parts.reduce((a, b) => a.replace(/\/+$/, "") + "/" + b.replace(/^\/+/, ""));
	...
	return {
		cwd: () => deno.cwd(),
		readdir: (dir) => [...deno.readDirSync(dir)].map((e: any) => e.name),
		lstat: (p) => {
			if (statCache.has(p)) return statCache.get(p)!;
...
function resolveGlob(fsAdapter: FSAdapter, pattern: string, opts: { cwd: string; dot: boolean; filesOnly: boolean }): string[] {
 ...
 */
function resolveAliases(pattern: string, aliasMap: AliasMap): string[] {
