...
Guards answer: *is this user allowed, and how should the query be scoped?*  
...
Effects answer: *after this commit happened, what should run?*  
...
export const DeactivationAlert = effect({
  ...
  run: async (data: InferModel<typeof UserEntity>) => {
    await SendEmailJob.dispatch({ to: data.email, subject: "Account deactivated" })
  },
})
```
...
export const AdminPanel = plugin((app) => {
  app.hook({ on: "HttpRequest", run: async (req, res) => {
...
import { UserEntity, OrgEntity }   from "./entities"
...
// Workflow
await client.workflow(OnboardingWorkflow).trigger({ userId: "u_1", videoPath: "..." })
```
