...
packages/mongo/
├── src/
│   ├── plugin.ts          # mongoDb() factory, registers adapter + models
│   ├── adapter.ts         # MongoDBAdapter (connect, collection, ensureIndex)
│   ├── model.ts           # Model<TDoc> base class + repository methods
│   ├── types.ts           # ModelDoc<T>, FieldType, InferSchema, HookContext
│   ├── builder.ts         # FieldChain, defineSchema proxy/builder
│   ├── hooks.ts           # HookRegistry, execution logic
│   ├── query.ts           # QueryBuilder implementation
│   ├── sync.ts            # Pipeline generator, batch processor, dry-run
│   └── seed.ts            # seedAll() utility
├── types/
│   └── index.d.ts         # Module augmentation for ctx.mongo (if used)
